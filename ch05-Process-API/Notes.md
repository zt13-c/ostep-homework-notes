...
## 题目 7：关闭标准输出 (STDOUT_FILENO) 会发生什么？

**题目要求：** 编写一个创建子进程的程序，然后在子进程中关闭标准输出（`STDOUT_FILENO`）。如果子进程在关闭描述符后调用 `printf()` 打印输出，会发生什么？
...
### 2. 运行结果
```
子进程：关闭 STDOUT 之前，你能看到这句话。
【调试信息】子进程 printf 返回值: -1
父进程：子进程执行完毕。
```
### 3. 实验分析与思考
```
通过这个实验，我们得到了明确的结论：在关闭 STDOUT_FILENO 后调用 printf()，字符串不会被打印到屏幕上，而是直接“消失”了。

深入剖析底层的原理：

1、printf 是个“打工人”： printf 并不是真正把字显示在屏幕上的函数，它只是 C 语言标准库（libc）提供的一个高级包装函数。它的底层实际上调用了操作系统的系统调用 write(1, ...)。这里的 1 就是 STDOUT_FILENO（标准输出文件描述符）的固定编号。

2、底层管道被切断： 默认情况下，文件描述符 1 指向你的终端屏幕（显示器）。当我们在代码里执行 close(STDOUT_FILENO) 时，操作系统无情地切断了 1 号管道和屏幕之间的连接。

3、数据黑洞： 之后我们再调用 printf()，printf 依然会傻傻地尝试去往 1 号管道里塞数据。但是操作系统一看，1 号管道已经关了，于是立刻拒绝接收，并返回一个错误码（所以我们在调试信息里看到 printf 的返回值是 -1，代表执行失败）。因为没有任何报错机制强制中断程序，所以这个错误被默默吞掉了，数据也就凭空消失了。

引申：
这个看似“破坏性”的动作，其实是 Linux 核心神技“输出重定向” (>) 的第一步！如果你在 close(STDOUT_FILENO) 之后，立刻 open() 一个新文件，那么这个新文件就会自动抢占刚刚空出来的 1 号位置。那时候你再调用 printf()，数据就不会上屏幕，而是全部写进那个新文件里了！

当你运行任何一个 C 程序时，操作系统会默认给你免费分配三个打开的文件描述符（管道）：

0 号管道 (stdin)：标准输入。默认连接键盘，用来接收你的敲击。

1 号管道 (stdout)：标准输出。默认连接屏幕，这是 printf 唯一指定的数据出口。

2 号管道 (stderr)：标准错误。默认也连接屏幕，专门用来输出报错和调试信息。

为什么用 fprintf 而不是 printf？
printf(...) 其实是个“残疾”函数，它底层被写死了，永远只能把数据塞进 1 号管道 (stdout)。

fprintf(目标, ...) 则是一个进化版（f 代表 file）。它允许你手动指定要把数据塞进哪个管道或文件里。

核心魔法：stderr
在第 7 题的代码中，我们刚刚执行了 close(STDOUT_FILENO);，亲手把 1 号管道（主大门）给焊死了。这时候再用 printf 往外发消息，全都会掉进黑洞。

既然主大门走不通，我们就走紧急通道！

stderr 就是那个 2 号管道。因为我们并没有关闭它，而且它默认也是通向你的终端屏幕的。所以，通过 fprintf 把数据塞进 stderr，这句话就能成功绕过被关闭的 1 号管道，顺利地显示在你的屏幕上。

"...", ret
这一部分就和普通的 printf 完全一样了。
```

## 题目 8：使用 pipe() 和 dup2() 实现进程间管道通信

**题目要求：** 编写一个程序，创建两个子进程，并使用 `pipe()` 系统调用，将一个子进程的标准输出连接到另一个子进程的标准输入。
...
### 2. 运行结果
```
子进程 2 成功接收到数据: 这是来自子进程 1 的机密情报，通过管道传输！
父进程：两个子进程已完成管道通信，圆满结束。
```
### 3. 实验分析与思考
```
本实验完美复刻了 Linux Shell 中管道符 | 的底层工作原理。其中有两个极其关键的系统机制：

dup2(oldfd, newfd) 狸猫换太子：

子进程 1 使用 dup2(fd[1], STDOUT_FILENO)。这会让系统的 1 号管道（标准输出）强行指向我们创建的物理管道的写端。从此，子进程 1 里所有向屏幕输出的操作（如 printf），都被偷偷拐进了管道。

子进程 2 使用 dup2(fd[0], STDIN_FILENO)。这会让系统的 0 号管道（标准输入）强行指向物理管道的读端。从此，子进程 2 原本以为在等键盘敲击（如 fgets、scanf），实际上是在等管道里流过来的数据。

为什么“关闭不用的管道描述符”关系到生死存亡？

管道的设计原则是：只要写端（fd[1]）还有一个引用没被关掉，读端（fd[0]）的 read 操作就会一直阻塞等待。

在 fork() 之后，父进程、子进程 1、子进程 2 手里都各自捏着一对 fd[0] 和 fd[1]。

如果父进程不执行 close(fd[1])，那么子进程 2 在读完数据后，会发现管道的写端还在被父进程握着。子进程 2 就会想：“父进程可能等下还要写数据，我再等等。”于是子进程 2 永远卡死，导致父进程的 waitpid 也永远卡死。这就引发了经典的死锁 (Deadlock)。
```
```
fd[1]（太子）：这是我们在代码开头用 pipe(fd) 建立的那个管道的写入端。你可以把它想象成一根水管的入口。

STDOUT_FILENO（狸猫）：这是 C 语言里预定义好的一个宏，它其实就是数字 1。在 Linux 里，1 号管道叫做“标准输出”，默认是指向你的终端屏幕的。printf 函数是个死脑筋，它永远只会把字写进 1 号管道。

dup2 的全称是 duplicate to（复制到）。它的语法是 dup2(旧的, 新的)。
它的执行逻辑极其暴力，包含两个隐蔽的连招：

先杀后借： 操作系统会先把后面的“新的”（也就是 1 号管道）残忍地关闭（切断它和屏幕的连接）。

强行克隆： 然后，操作系统会让 1 号管道强行指向前面那个“旧的”（也就是 fd[1]）所指向的地方。

当你执行了 dup2(fd[1], STDOUT_FILENO);，操作系统在底层完成了一次瞒天过海的管道重定向：
“拔掉原来通向屏幕的 1 号水管，把它直接对准我们刚刚创建的物理管道的入口（fd[1]）！”

执行 dup2(fd[1], STDOUT_FILENO) 后，你的进程表里就有两个位置（一个是 fd[1]，一个是 1）同时指向了管道的同一个入口。

为什么要关掉原本的 fd[1]？主要有两个原因：一个是“洁癖”（资源管理），另一个是“生死攸关”（避免死锁）。

1. 资源管理的“洁癖”
文件描述符是有限的资源（虽然现代系统很多，但也是有限的）。dup2 就像是做了一次“克隆”。克隆完成后，1 号（STDOUT）已经成功接管了往管道写数据的任务。
此时，原本的 fd[1] 就变成了“冗余”的占位符。把它关掉可以保持进程文件描述符表的整洁，防止长时间运行的程序出现“文件描述符泄漏”。

2. 核心原因：防止“幽灵写者”导致的死锁（EOF 问题）
这是最关键的原因。在管道通信中，读端（Reader） 判定数据是否读完的唯一标准是：是否收到了 EOF（文件结束符）。

操作系统规定： 只有当所有指向管道写端的文件描述符都关闭时，内核才会向读端发送 EOF 信号。

现在的子进程 2 只读 一行，不是一直读到 EOF。
...
因此它不会等到管道 EOF，自然也就不会死锁。

真正会死锁的是这种情况：子进程 2 想一直读，直到读到 EOF 为止。
...
while (fgets(buffer, sizeof(buffer), stdin) != NULL) {
    printf("子进程 2 接收到: %s", buffer);
}

这时 fgets() 读完第一行后，还会继续读下一行。

但是现在管道里已经没数据了，而父进程还没有关闭 fd[1] 写端。
